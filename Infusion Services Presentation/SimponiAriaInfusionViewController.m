//
//  SimponiAriaInfusionViewController.m
...
@interface SimponiAriaInfusionViewController ()<IOMAnalyticsIdentifiable>
{
    SimponiAriaInfusion* _simponiAriaInfusion;
...
@implementation SimponiAriaInfusionViewController
...
- (void)viewDidLoad
{
    ...
    self.textFieldArray = @[_averageInfusionsPerMonthTextField, _averageNewPatientsPerMonthQ8TextField];
...
- (SimponiAriaInfusion*)simponiAriaInfusion
{
    if(!_simponiAriaInfusion){
        //Get simponi infusion model to display and modify
        _simponiAriaInfusion = [SimponiAriaInfusion getSimponiAriaInfusionForScenario:_scenario];
    }
    
    return _simponiAriaInfusion;
}
...
- (void)initializeInputData
{
    ...
    NSString* avgInfusionsPerMonthString = ([[self simponiAriaInfusion].avgInfusionsPerMonth intValue] == 0) ? @"" : [[self simponiAriaInfusion].avgInfusionsPerMonth stringValue];
    NSString* avgNewQ6String = ([[self simponiAriaInfusion].avgNewPatientsPerMonth intValue] == 0) ? @"" : [[self simponiAriaInfusion].avgNewPatientsPerMonth stringValue];
    
    [_averageInfusionsPerMonthTextField setText:avgInfusionsPerMonthString];
    [_averageNewPatientsPerMonthQ8TextField setText:avgNewQ6String];
...
{
    [_preInfusionTimeButton setTitle:[NSString stringWithFormat:@"%i", [[self simponiAriaInfusion].prepTime intValue] *10] forState:UIControlStateNormal];
    [_postInfusionTimeButton setTitle:[NSString stringWithFormat:@"%i", [[self simponiAriaInfusion].postTime intValue] *10] forState:UIControlStateNormal];
}

- (void)updateAncillarySelections
{
    [_preInfusionAncillarySwitch setOn:[[self simponiAriaInfusion].prepAncillary boolValue]];
    [_postInfusionAncillarySwitch setOn:[[self simponiAriaInfusion].postAncillary boolValue]];
}
...
- (IBAction)ancillarySwitchChanged:(id)sender
{
    if(sender == _preInfusionAncillarySwitch){
        [self simponiAriaInfusion].prepAncillary = [NSNumber numberWithBool:_preInfusionAncillarySwitch.isOn];
    }else if(sender == _postInfusionAncillarySwitch){
        [self simponiAriaInfusion].postAncillary = [NSNumber numberWithBool:_postInfusionAncillarySwitch.isOn];
    }
}

- (IBAction)clearButtonSelected:(id)sender
{
    [self simponiAriaInfusion].prepTime = [NSNumber numberWithInt:DEFAULT_SIMPONI_ARIA_PREP1_TIME + DEFAULT_SIMPONI_ARIA_PREP2_TIME];
    [self simponiAriaInfusion].postTime = [NSNumber numberWithInt:DEFAULT_SIMPONI_ARIA_POST1_TIME + DEFAULT_SIMPONI_ARIA_POST2_TIME];
    [self simponiAriaInfusion].prepAncillary = [NSNumber numberWithBool:NO];
    [self simponiAriaInfusion].postAncillary = [NSNumber numberWithBool:NO];
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    if(textField == _averageInfusionsPerMonthTextField){
        [self simponiAriaInfusion].avgInfusionsPerMonth = [NSNumber numberWithInt:[textField.text intValue]];
    }else if(textField == _averageNewPatientsPerMonthQ8TextField){
        [self simponiAriaInfusion].avgNewPatientsPerMonth = [NSNumber numberWithInt:[textField.text intValue]];
    }
}
...
- (void)createDropdownDataSources
{
    _prepTimeDropdownDataSource = [[DropdownDataSource alloc]
                                      initWithItems:LIST_VALUES_ARRAY_SIMPONI_PREP_TIME
    ...
    _postTimeDropdownDataSource = [[DropdownDataSource alloc]
                                       initWithItems:LIST_VALUES_ARRAY_SIMPONI_POST_TIME
                                   andTitleForItemBlock:^ NSString *(id item){
                                       return [NSString stringWithFormat:@"%i", [item intValue] * 10];
                                       }];
}
...
- (void)dropdown:(DropdownController *)dropdown item:(id)item selectedAtIndex:(int)index fromDataSource:(DropdownDataSource *)dataSource
{
    if(dataSource == _prepTimeDropdownDataSource){
        //Set new preptime
        [self simponiAriaInfusion].prepTime = [NSNumber numberWithInt:[item intValue]];
        [self updateStepTimes];
    }else if(dataSource == _postTimeDropdownDataSource){
        [self simponiAriaInfusion].postTime = [NSNumber numberWithInt:[item intValue]];
        [self updateStepTimes];
    }
